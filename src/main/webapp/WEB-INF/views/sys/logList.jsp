...
<body>
	...
	<form:form id="searchForm" cssclass="breadcrumb form-search" method="post"
			action="${ctx}/sys/log/list">
		...
		<div>
			...
			<input id="title" name="title"
						type="text" maxlength="50" class="input-mini" value="${log.title}"/>
			<label>用户ID：</label>
				<input id="createBy.id" name="createBy.id" type="text"
					   maxlength="50" class="input-mini" value="${log.createBy.id}"/>
			<label>URI：</label><input id="requestUri" name="requestUri"
									  type="text" maxlength="50" class="input-mini" value="${log.requestUri}"/>
			...
			&nbsp;<label for="type"><input id="type" name="type" type="checkbox" value="2" ${not empty log.type ? 'checked' : ''} />只查询异常信息</label>
			...
			<c:if test="${not empty log.exception}">
				<tr>
					<td colspan="8" style="word-wrap:break-word;word-break:break-all;">
						异常信息: <br/>
							${fn:replace(fn:replace(log.exception, strEnter, '<br/>'), strTab, '&nbsp; &nbsp; ')}
					</td>
				</tr>
			</c:if>
		</c:forEach>
	...
	<div class="pagination">${pm:pageStr(pageInfo)}</div>
</body>
</html>